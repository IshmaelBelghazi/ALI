...
through semi-supervised benchmarks on SVHN and CIFAR10. We adapt the
discriminative model proposed in \citet{Salimans2016gan} to take both $x$ and
$z$ as input.
...
\autoref{tab:semi-supervised}.

\begin{table}[ht]
\caption{\label{tab:semi-supervised} SVHN and CIFAR10 test set missclassification rate for
	semi-supervised learning using different numbers of trained labeled examples.}
\centering
\scalebox{0.75}{
\begin{tabular}{@{}ll|llll@{}} \toprule
 & \textbf{SVHN} & \multicolumn{4}{|c}{\textbf{CIFAR10}} \\  
Number of labeled examples   & 1000  & 1000 & 2000 & 4000 & 8000    \\ \midrule
...
\textbf{GAN (feature matching) \citep{Salimans2016gan}} & $\mathbf{8.11 \pm 1.3}$ & $\mathbf{21.83 \pm 2.01}$ & $\mathbf{19.61 \pm 2.09}$ & $\mathbf{18.63 \pm 2.32}$ & $\mathbf{17.72 \pm 1.82}$ \\ 
\textbf{ALI (ours, no feature matching)} & $\textbf{7.3}$ & $\textbf{20.88}$ & $\textbf{20.28}$ & $\textbf{18.3}$ & $\textbf{17.77}$ \\
\bottomrule
\end{tabular}
}
\vspace{0.2cm}
