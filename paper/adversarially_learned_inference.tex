...
GAN-based convolutional~\citep{radford2015unsupervised,denton2015deep} and
recurrent~\citep{im2016generating} generative networks have been able to achieve
dramatic improvements in the quality of synthetic images.
...
\begin{prop}
    At optimality, the encoder and decoder are inverses of one another.
\end{prop}
\begin{proof}
    The proof is given in \citep{donahue2016adversarial}, in which the authors
...
\begin{figure}[t]
    \centering
    \includegraphics[height=\figureHeight]{celeba_interpolations.png}
    \caption{\label{fig:celeba_interpolations} Latent space interpolations on
        ...
        interpolated linearly from $\mathbf{z}_1$ to $\mathbf{z}_2$.}
\end{figure}
...
DCGAN + L2-SVM \citep{radford2015unsupervised} & $22.18\% (\pm 1.13\%)$          \\
{\bf SDGM \citep{maaloe2016auxiliary}}         & $\mathbf{16.61\% (\pm 0.24\%)}$ \\ \midrule
...
The fact that ALI's latent representation tends to focus on semantic information
at the expense of low-level details leads us to believe that ALI may be well
suited to semi-supervised tasks. We empirically verify this by achieving a
competitive performance on the semi-supervised SVHN classification task.
Results are presented in \autoref{tab:semi-supervised}. The SDGM's performance
is achieved via a carefully designed two-layer architecture that explicitly
takes label information into account in learning the representation. We expect
that ALI would also gain by taking account of label information in learning the
representation.
...
shown to be useful, achieving competitive results on semi-supervised SVHN house
